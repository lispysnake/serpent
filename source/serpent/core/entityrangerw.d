...
module serpent.core.entityrangerw;
...
/**
 * Convert C into a pointer, unless its an EntityIdentifier field,
 * in which case it is ALWAYS const. This is to prevent accidental
 * modification of the entity ID, which would break the world.
 */
template PtrToCConditional(C)
{
    static if (is(C == EntityIdentifier))
    {
        alias PtrToCConditional = const(C*);
    }
    else
    {
        alias PtrToCConditional = C*;
    }
}
 ...
 * super range.
 */
package struct EntityRangeRW(C...)
{
private:
    ...
    else
    {
        ...
        alias ComponentSetCast = Tuple!(staticMap!(PtrToCConditional, C));
    }
